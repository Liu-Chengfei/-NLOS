...
这个模块只负责把结构化窗口送进 Transformer，然后把网络的四个固定输出头整理成
...
from liquidloc.protocol.risk_projection import project_risk_to_protocol_range  # §12.3-C2a 第十四轮：risk 投影权威单源，与 LSTM/Liquid 路径一致委托。
...
from liquidloc.models.transformer.network import normalize_structured_window  # 复用同一套结构化窗口校验逻辑。
...
def infer_intermediate(  # 对结构化窗口执行一次推理并整理输出。
    ...
    """对一个结构化窗口执行一次 Transformer 推理，并整理成中间量对象。
    ...
    }, "transformer.infer_intermediate")
    ...
    if hasattr(model, 'risk_calibration') and callable(getattr(model, 'risk_calibration', None)):
        ...
        risk = float(1.0 / (1.0 + float(torch.tensor(-raw_risk_tensor.detach().cpu().item()).exp().item())))  # 无校准模块时用简单 sigmoid。
