"""Transformer 模块对外统一入口。

这个文件不实现模型本身，只负责把 `transformer` 子目录里真正给外部使用的
...
from liquidloc.models.transformer.inference import infer_intermediate  # 导出 Transformer 中间量推理函数，给外部统一调用。
from liquidloc.models.transformer.network import (  # 导出网络结构相关工具，供训练、推理和配置解析共用。
    TransformerNetwork,  # 导出真正的 Transformer 网络类，供工厂、训练器和推理器实例化。
    build_transformer_sequence_tensor,  # 导出结构化窗口转序列张量的构造函数，保证输入形状一致。
    normalize_structured_window,  # 导出结构化窗口标准化函数，供前后端统一校验字段。
    resolve_transformer_network_cfg,  # 导出网络配置解析函数，供工厂和测试复用配置约定。
)
...
__all__ = (  # 定义该包对外允许显式导出的符号，避免外部误依赖内部私有实现。
    "TransformerNetwork",  # 对外暴露 Transformer 网络类。
    "build_transformer_sequence_tensor",  # 对外暴露结构化窗口转序列张量函数。
    "infer_intermediate",  # 对外暴露中间量推理函数。
    "normalize_structured_window",  # 对外暴露结构化窗口标准化函数。
    "resolve_transformer_network_cfg",  # 对外暴露 Transformer 配置解析函数。
